...
Features:
- Physical Force Disturbance Injection via MuJoCo qfrc_applied.
- Mathematically rigorous Potential-Based Reward Shaping (PBRS) with terminal zero-potential boundary.
- Per-episode reset of Lagrangian dual multipliers to avoid vectorized state contamination.
- Extended Angle Failure Limits (45°), Cart Track Position Boundary Enforcement ([-1.5m, +1.5m]).
- Parallel multiprocessing via SubprocVecEnv.
"""
...
class WideCatchDisturbanceWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        # Disable disturbances in eval for clean benchmark score
        ...
        # Expand logical Gym action space so PPO outputs up to 20N
        ...
        self.unwrapped.action_space = new_space
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.is_eval:
            # Standard upright start for clean evaluation metrics
            random_angle = 0.0
            random_omega = 0.0
        else:
            ...
            # 80% chance pole velocity swings towards upright
        ...
        # Clear external physics forces & disturbance state
        unwrapped.data.qfrc_applied[:] = 0.0
        self._push_remaining = 0
        self._push_force = 0.0

        # Reset adaptive dual weight per episode to prevent bleed-over across vectorized runs
        self.lambda_weight = 0.0

        # Initialize potential tracking for the new state
    ...
    def step(self, action):
        ...
        # Multi-step impulse disturbance setup
        if self._push_remaining == 0 and random.random() < self.disturbance_prob:
            self._push_force = random.uniform(-self.max_push, self.max_push)
            self._push_remaining = random.randint(2, 5)

        # Apply external force directly to the cart joint (DOF 0) via MuJoCo qfrc_applied
        if self._push_remaining > 0:
            unwrapped.data.qfrc_applied[0] = self._push_force
            self._push_remaining -= 1
        else:
            unwrapped.data.qfrc_applied[0] = 0.0

        # Step environment using the pure, unmodified policy action
        clipped_action = np.clip(action, -20.0, 20.0)
        obs, _, _, truncated, info = self.env.step(clipped_action)

        # Clear applied external forces after the physics solver step
        unwrapped.data.qfrc_applied[0] = 0.0
        ...
        # Strict boundary condition: Terminal state potential is zero (Ng et al., 1999)
        if terminated:
        ...
        # Safe RL Constraints & Primal-Dual update
        cost = 0.0
        ...
        # Base step reward: 1.0 if surviving, 0.0 if terminated (no ad-hoc penalty)
        base_step_reward = 0.0 if terminated else 1.0
        R_penalized = base_step_reward + F_shaping - (self.lambda_weight * cost)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--disturbance-prob", type=float, default=0.08)
    parser.add_argument("--max-push", type=float, default=4.0)
    parser.add_argument("--angle-weight", type=float, default=3.0)
...
def main() -> None:
    ...
    eval_raw_env = gym.make(ENV_ID, render_mode="human")
    eval_env = Monitor(

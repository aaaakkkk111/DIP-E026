...
static void RouteA_ServiceBluetooth(void)
{
	if(newLineReceived)
	{
		ProtocolCpyData();
		if(RouteA_IsFrame(ProtocolString))
		{
			RouteA_HandleFrame((char *)ProtocolString);
			newLineReceived = 0;
...
{	
	...
	if(mode == Test_Mode) RouteA_BalanceStarted();
	...
	while(1)
	{
		if(mode >= LiDar_avoid && mode != Test_Mode)//开启雷达模式 接收的数据 //Activate radar mode to receive data
		{
		...
		if(mode == Normal || mode == Weight_M)//Official modes 1 and 4 remain unchanged.
		{
			if (newLineReceived)
			{
				ProtocolCpyData();
		...
		else if(mode == Test_Mode)
		{
			...
			sprintf(showbuf,"TEST A=%d       ",(int)Angle_Balance);
			OLED_Draw_Line(showbuf, 3, false, true);
		}

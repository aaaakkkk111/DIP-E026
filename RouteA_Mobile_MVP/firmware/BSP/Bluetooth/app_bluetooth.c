...
//	/*防止数据丢包 Preventing Data Loss */
		/* Route A training lock also protects the official legacy PID write/restore path. */
	if(mode == Test_Mode && RouteA_ManualLocked() && (ProtocolString[5]=='2' || ProtocolString[9]=='1' ||
		ProtocolString[11]=='1' || ProtocolString[13]=='1'))
	{
		newLineReceived = 0;
		memset(ProtocolString, 0x00, sizeof(ProtocolString));
		UART5_Send_Char("$TrainingLocked#");
		return;
	}

...
int Voltage,Mid_Angle;                          		//电池电压采样相关的变量、机械中值 Battery voltage sampling related variables, mechanical median
...
{	
	...
	while(1)
	{
		...
		if(bulettohflag == 1) //此方法上报的数据，app会出现bug The data reported by this method may cause a bug in the app
		{
			bulettohflag = 0;

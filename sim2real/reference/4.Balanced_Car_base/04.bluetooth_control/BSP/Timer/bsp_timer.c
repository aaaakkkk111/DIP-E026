...
void TIM6_IRQHandler(void)
{
	if (TIM_GetITStatus(TIM6, TIM_IT_Update) != RESET) //检查TIM更新中断发生与否  Check whether TIM update interruption occurs
	{
		...
		Get_Distane();//获取超声波距离 	Get distance
			bulettohflag = 1;	 //蓝牙自动上报数据标志 放到mian函数	Bluetooth automatic data reporting flag is placed in the main function
//		SendAutoUp();//蓝牙自动上报数据 Bluetooth automatic data reporting

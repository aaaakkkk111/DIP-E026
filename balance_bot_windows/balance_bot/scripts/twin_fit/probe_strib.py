...
CASES = [("基线", {})]
for fs in (0.001, 0.0025, 0.005):
    for fkr in (0.2, 0.6):
        for vs in (0.3, 2.0):
            CASES.append(("fs%.4f k%.1f v%.1f" % (fs, fkr, vs), dict(fs=fs, fkr=fkr, vs=vs)))
for fs, dd in ((0.005, 20), (0.01, 40), (0.01, 70)):
    CASES.append(("fs%.3f 死区-%d" % (fs, dd), dict(fs=fs, fkr=0.3, vs=0.5, dpos=BASE["dpos"] - dd, dneg=BASE["dneg"] - dd)))

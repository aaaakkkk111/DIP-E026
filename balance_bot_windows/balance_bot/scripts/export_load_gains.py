# -*- coding: utf-8 -*-
"""把负载自动切档的增益和检测器导成可以直接抄进板子的 C。

用法：  python scripts/export_load_gains.py [输出路径]
"""
import os
import sys
...
from balance_bot.firmware.load_sched import (NORMAL, HEAVY,       # noqa: E402
                                             FACTORY)
...
def gains_literal(mode):
    g = mode.gains()
    return "{ " + ", ".join(f"{g[k] * 100.0:.1f}f" for k in ORDER) + " }"
...
static const LoadGains G_NORMAL = {g_normal};
static const LoadGains G_HEAVY  = {g_heavy};
...
/* ==== 2. 检测器：200 Hz 中断里调一次 ==== */
/* {lo_hz:.1f}~{hi_hz:.1f} Hz 带通 -> 平方 -> 低通。两级一阶滤波，4 个乘加。 */

#define LD_HZ           {fs:.1f}f
#define LD_A_HP         {a_hp:.6f}f   /* 高通拐点 {lo_hz:.1f} Hz */
#define LD_A_LP         {a_lp:.6f}f   /* 低通拐点 {hi_hz:.1f} Hz */
#define LD_A_EN         {a_en:.6f}f   /* 能量平滑 */
#define LD_LSB_PER_DPS  {lsb:.1f}f

/* 下面两个数是孪生里的，**上车必须重标**，见第 4 节 */
float LD_LOADED_BELOW = {loaded:.1f}f;   /* NORMAL 下低于此 -> 装了货 */
float LD_EMPTY_ABOVE  = {empty:.1f}f;   /* HEAVY  下高于此 -> 车空了 */

#define LD_PRIME_S      {prime:.2f}f   /* 预热，之前不判决 */
#define LD_CONFIRM_S    {confirm:.1f}f    /* 判定要连续成立这么久 */
#define LD_LOCKOUT_S    {lockout:.1f}f    /* 换档后禁判，防止边界来回跳 */
...
{{
    ...
    ld_mean   += LD_A_HP * (gyro - ld_mean);      /* 去掉 {lo_hz:.0f} Hz 以下 */
    hp         = gyro - ld_mean;
    ld_band   += LD_A_LP * (hp - ld_band);        /* 去掉 {hi_hz:.0f} Hz 以上 */
...
/* ==== 5. 原厂值，对照用 ==== */
/* static const LoadGains G_FACTORY = {g_factory}; */
...
def main(path):
    src = HEAD.format(
        ...
        g_normal=gains_literal(NORMAL),
        g_heavy=gains_literal(HEAVY),
        g_factory=gains_literal(FACTORY),
        fs=FS,
        a_hp=alpha(D.HP_TAU), a_lp=alpha(D.LP_TAU), a_en=alpha(D.ENERGY_TAU),
        lo_hz=1.0 / (2 * 3.14159265 * D.HP_TAU),
        hi_hz=1.0 / (2 * 3.14159265 * D.LP_TAU),
        lsb=D.GYRO_LSB_PER_DEG_S,
        loaded=D.LOADED_BELOW, empty=D.EMPTY_ABOVE,
        prime=3.0 * D.ENERGY_TAU, confirm=D.CONFIRM_SECONDS,
        lockout=D.LOCKOUT_SECONDS)
    with open(path, "w", encoding="utf-8") as f:
        f.write(src)
    print(f"写入 {path}  ({src.count(chr(10)) + 1} 行)")
    for tag, m in (("NORMAL", NORMAL), ("HEAVY", HEAVY)):
        g = m.gains()
        print(f"  {tag:7s} Kp={g['balance_kp']*100:.0f} Kd={g['balance_kd']*100:.0f}"
              f" Vkp={g['velocity_kp']*100:.0f} Vki={g['velocity_ki']*100:.0f}")
    print(f"  阈值    LOADED_BELOW={D.LOADED_BELOW} EMPTY_ABOVE={D.EMPTY_ABOVE}"
          f"   带通 {1/(2*3.14159265*D.HP_TAU):.1f}-"
          f"{1/(2*3.14159265*D.LP_TAU):.1f} Hz")


if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "load_gains.c")
